...
declare
    ...
    statement_row_i_d text = format(
        'drop trigger if exists audit_i_d on %s;',
        $1
    );
    
    statement_row_u text = format(
        'drop trigger if exists audit_u on %s;',
        $1
    );
...
begin
    execute statement_row;
    execute statement_row_i_d;
    execute statement_row_u;
    execute statement_stmt;
end;
$$;

-- enable_selective_tracking accepts a relation and a list of columns which will be tracked.
create or replace function audit.enable_selective_tracking(regclass, tracked_columns text[])
    returns void
    volatile
    security definer
    set search_path = ''
...
begin
    ...
    if not exists(select 1 from pg_trigger where tgrelid = $1 and tgname = 'audit_i_d') then
        execute statement_row_i_d;
    end if;
    
    if not exists(select 1 from pg_trigger where tgrelid = $1 and tgname = 'audit_u') then
        execute statement_row_u;
    end if;
